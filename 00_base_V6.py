...
def collect_ingredients(number):
    ...
    for i in range(number):
        ...
        ingredient_name = not_blank("What is ingredient {}'s name: ".format(i+1),
                                    "sorry this can't be blank, please enter the ingredient's name")
...
if __name__ == "__main__":
    ...
    for i in ingredient_list:
        print(f"{i.name}, {i.needed}, {i.purchased}, {i.cost}, {i.cost_used()}")
        total_cost += i.cost_used()

    print("{:.2f}".format(total_cost))
    cost_per_serving = total_cost / serving_size
    print("{:.2f}".format(cost_per_serving))
